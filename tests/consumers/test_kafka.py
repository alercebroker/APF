...
from apf.consumers import KafkaConsumer, KafkaJsonConsumer
import unittest
...
class KafkaConsumerMock(unittest.TestCase):
    ...
    def consume(self, num_messages=1, timeout=60):
        messages = [MessageMock(self.error, self.eof_error)] * num_messages
        return messages
...
class TestKafkaConsumer(GenericConsumerTest, unittest.TestCase):
    ...
    def test_consume(self, mock_consumer):
        ...
        for msj in comp.consume():
            ...
            comp.commit()
            comp.consumer.eof_error = True
        self.assertIsNone(comp.messages)

    @mock.patch(
        "apf.consumers.kafka.Consumer",
        return_value=KafkaConsumerMock(),
    )
...
class TestKafkaJsonConsumer(unittest.TestCase):
    ...
    def test_deserialize(self):
        ...
        deserialized = consumer._deserialize_message(msg)
